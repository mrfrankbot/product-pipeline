...
const seedDefaultMappings = async (sqlite) => {
 ...
 */
const seedDefaultSettings = (sqlite) => {
 ...
 */
const migrateProductMappings = (sqlite) => {
    const cols = sqlite.prepare(`PRAGMA table_info(product_mappings)`).all();
 ...
 */
const migrateProductDraftsEbay = (sqlite) => {
    const cols = sqlite.prepare(`PRAGMA table_info(product_drafts)`).all();
    const colNames = new Set(cols.map((c) => c.name));
 ...
 */
const migrateProductNotes = (sqlite) => {
    const cols = sqlite.prepare(`PRAGMA table_info(product_mappings)`).all();
    const colNames = new Set(cols.map((c) => c.name));
 ...
 */
const migrateHelpQuestions = (sqlite) => {
    const cols = sqlite.prepare(`PRAGMA table_info(help_questions)`).all();
    const colNames = new Set(cols.map((c) => c.name));
 ...
 */
const seedGettingStartedContent = (sqlite) => {
    const insert = sqlite.prepare(`
...
The Dashboard auto-refreshes every 15 seconds, so you're always seeing current data. For deeper analytics, check the **Analytics** page.`,
    ...
    const updateSort = sqlite.prepare(`UPDATE help_questions SET sort_order = ? WHERE question = ? AND sort_order = 0`);
 ...
 */
const migrateHelpCategoryCase = (sqlite) => {
    sqlite.exec(`
...
const migrateProductDraftsTags = (sqlite) => {
    const cols = sqlite.prepare(`PRAGMA table_info(product_drafts)`).all();
    const colNames = new Set(cols.map((c) => c.name));
...
const migrateNotificationLog = (sqlite) => {
    const cols = sqlite.prepare(`PRAGMA table_info(notification_log)`).all();
    const colNames = new Set(cols.map((c) => c.name));
