...
function getProviderConfig() {
    const raw = (process.env.IMAGE_PROCESSOR ||
        process.env.IMAGE_SERVICE ||
        'auto').toLowerCase();
    // Normalize aliases
    if (raw === 'local' || raw === 'self-hosted')
        return 'self-hosted';
    if (raw === 'photoroom')
        return 'photoroom';
    return 'auto';
...
export async function getImageService() {
    ...
    if (mode === 'photoroom') {
        if (!apiKey)
            throw new Error('IMAGE_PROCESSOR=photoroom but PHOTOROOM_API_KEY not set');
 ...
 */
export async function timedImageCall(label, fn) {
