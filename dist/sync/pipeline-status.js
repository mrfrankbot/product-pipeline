...
const toEpochSeconds = (iso) => iso ? Math.floor(new Date(iso).getTime() / 1000) : undefined;
...
const persistJob = async (job) => {
    ...
    db.prepare(`INSERT INTO pipeline_jobs
     ...
     ON CONFLICT(id) DO UPDATE SET
      ...
      updated_at = excluded.updated_at`).run(job.id, job.shopifyProductId, job.shopifyTitle ?? null, job.status, job.currentStep ?? null, serializeSteps(job.steps), toEpochSeconds(job.startedAt), toEpochSeconds(job.completedAt), job.error ?? null, createdAt, updatedAt);
};
// ---------------------------------------------------------------------------
// Public API
// ---------------------------------------------------------------------------
/** Create a new pipeline job and return its ID. */
export async function createPipelineJob(shopifyProductId) {
...
export function getPipelineJob(id) {
    return jobs.get(id);
}
/** Mark the overall job as processing. */
export async function startPipelineJob(jobId) {
...
export async function updatePipelineStep(jobId, stepName, status, result) {
    const job = jobs.get(jobId);
    if (!job)
        return;
...
export function emitProgress(jobId, stepName, current, total, detail) {
    const job = jobs.get(jobId);
    if (!job)
        return;
    const event = {
...
export async function setPipelineJobTitle(jobId, title) {
    const job = jobs.get(jobId);
    if (!job)
        return;
...
export function cancelPipelineJob(jobId) {
    const job = jobs.get(jobId);
    if (!job)
        return false;
...
export function cleanupStuckJobs() {
