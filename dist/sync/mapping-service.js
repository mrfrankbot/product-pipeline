 ...
 */
export const getMapping = async (mappingType, sourceValue) => {
    ...
    if (sourceValue) {
        const exactMatch = await db
            .select()
            .from(fieldMappings)
            .where(and(eq(fieldMappings.mappingType, mappingType), eq(fieldMappings.sourceValue, sourceValue)))
            .get();
        if (exactMatch) {
        ...
        if (mappingType === 'condition' || mappingType === 'category') {
            const allMappings = await db
                ...
                .all();
            const partialMatch = allMappings.find(mapping => mapping.sourceValue &&
    ...
    const defaultMapping = await db
        .select()
        .from(fieldMappings)
        .where(and(eq(fieldMappings.mappingType, mappingType), eq(fieldMappings.isDefault, true)))
        .get();
    if (defaultMapping) {
 ...
 */
export const mapCondition = async (tags) => {
 ...
 */
export const mapCategory = async (productType) => {
 ...
 */
export const mapField = async (shopifyFieldName) => {
 ...
 */
export const getMappingsByType = async (mappingType) => {
    const db = await getDb();
    return db
        .select()
        .from(fieldMappings)
...
export const getAllMappings = async () => {
    const db = await getDb();
    const allMappings = await db.select().from(fieldMappings).all();
    const grouped = {};
 ...
 */
export const createMapping = async (mappingType, sourceValue, targetValue, isDefault = false) => {
    const db = await getDb();
    ...
    const newMapping = await db
        .select()
        .from(fieldMappings)
        .where(eq(fieldMappings.id, result.lastInsertRowid))
        .get();
    return newMapping;
};
/**
...
export const updateMapping = async (id, updates) => {
    ...
    if (updates.isDefault) {
        await db
            .update(fieldMappings)
            .set({ isDefault: false })
            .where(and(eq(fieldMappings.mappingType, existing.mappingType), eq(fieldMappings.isDefault, true)))
            .run();
    }
    await db
        .update(fieldMappings)
