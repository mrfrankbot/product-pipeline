...
router.post('/webhooks/ebay/notifications', async (req, res) => {
    ...
    try {
        ...
        const notificationType = body?.GetItemTransactionsResponse?.NotificationType ||
        ...
        db.prepare(`INSERT INTO notification_log (source, topic, message) VALUES (?, ?, ?)`).run('ebay', notificationType || 'unknown', rawBody.substring(0, 10000));
        if (notificationType) {
...
function findNotificationType(obj) {
...
async function handleNotification(type, _parsed, _raw) {
    switch (type) {
    ...
    db.prepare(`UPDATE notification_log SET processed_at = unixepoch()
     WHERE source = 'ebay' AND topic = ? AND id = (SELECT id FROM notification_log WHERE source = 'ebay' AND topic = ? ORDER BY id DESC LIMIT 1)`).run(type, type);
}
