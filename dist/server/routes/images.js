...
async function fetchShopifyProductImages(accessToken, productId) {
    const res = await fetch(`https://usedcameragear.myshopify.com/admin/api/2024-01/products/${productId}/images.json`, { headers: { 'X-Shopify-Access-Token': accessToken } });
    if (!res.ok) {
...
async function uploadProcessedImageToShopify(accessToken, productId, imageBuffer, filename) {
    const base64 = imageBuffer.toString('base64');
    const res = await fetch(`https://usedcameragear.myshopify.com/admin/api/2024-01/products/${productId}/images.json`, {
        method: 'POST',
        headers: {
...
// ────────────────────────────────────────────────────────────────────────
router.get('/api/products/:id/images', async (req, res) => {
    try {
        ...
        const db = await getRawDb();
        const logRows = db.prepare(`SELECT * FROM image_processing_log WHERE product_id = ? ORDER BY created_at DESC`).all(productId);
        // Build a map from original URL to latest processing entry
        const processingByUrl = new Map();
...
// ────────────────────────────────────────────────────────────────────────
router.post('/api/products/:id/images/reprocess', async (req, res) => {
    try {
        const productId = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
       ...
       VALUES (?, ?, ?, 'processing', ?, ?, ?)`).run(productId, imageUrl, imageUrl, JSON.stringify(params), now, now);
        const logId = insertResult.lastInsertRowid;
        info(`[Images API] Reprocessing image for product ${productId} (log ${logId})`);
        try {
            const imageService = await getImageService();
            const { dataUrl } = await timedImageCall(`reprocess product=${productId}`, () => imageService.processWithParams(imageUrl, params));
            ...
            if (accessToken) {
                try {
                    ...
                    shopifyImage = await uploadProcessedImageToShopify(accessToken, productId, buffer, `processed-${Date.now()}.png`);
...
// ────────────────────────────────────────────────────────────────────────
router.post('/api/products/:id/images/reprocess-all', async (req, res) => {
    try {
        const productId = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
        ...
        for (const img of shopifyImages) {
            const now = Math.floor(Date.now() / 1000);
            const insertResult = db.prepare(`INSERT INTO image_processing_log (product_id, image_url, original_url, status, params_json, created_at, updated_at)
         VALUES (?, ?, ?, 'processing', ?, ?, ?)`).run(productId, img.src, img.src, JSON.stringify(params), now, now);
            const logId = insertResult.lastInsertRowid;
            try {
                const { buffer, dataUrl } = await timedImageCall(`reprocess-all product=${productId} image=${img.id}`, () => imageService.processWithParams(img.src, params));
                db.prepare(`UPDATE image_processing_log SET status = 'completed', processed_url = ?, updated_at = ? WHERE id = ?`).run(dataUrl, Math.floor(Date.now() / 1000), logId);
                // Upload to Shopify
                let shopifyImageId;
                try {
                    const shopifyImage = await uploadProcessedImageToShopify(accessToken, productId, buffer, `processed-${img.id}-${Date.now()}.png`);
        ...
        db.prepare(`INSERT INTO product_pipeline_status (shopify_product_id, images_processed, images_processed_count, created_at, updated_at)
       VALUES (?, 1, ?, ?, ?)
       ON CONFLICT(shopify_product_id) DO UPDATE SET
         images_processed = 1,
         images_processed_count = excluded.images_processed_count,
         updated_at = excluded.updated_at`).run(productId, succeeded, Math.floor(Date.now() / 1000), Math.floor(Date.now() / 1000));
        res.json({
            ok: true,
            productId,
...
// ────────────────────────────────────────────────────────────────────────
router.get('/api/products/:id/images/:imageId/status', async (req, res) => {
    try {
        const productId = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
        ...
        let logRow = db.prepare(`SELECT * FROM image_processing_log WHERE id = ? AND product_id = ?`).get(imageId, productId);
        if (!logRow) {
            // Try to find by matching Shopify image URL pattern
            const rows = db.prepare(`SELECT * FROM image_processing_log WHERE product_id = ? ORDER BY created_at DESC`).all(productId);
            logRow = rows.find((r) => r.image_url.includes(imageId) || r.original_url.includes(imageId));
        }
        if (!logRow) {
            res.json({
                ok: true,
...
router.delete('/api/products/:productId/images/:imageId', async (req, res) => {
    try {
        ...
        // Delete from Shopify
        const deleteResponse = await fetch(`https://usedcameragear.myshopify.com/admin/api/2024-01/products/${productId}/images/${imageId}.json`, {
            method: 'DELETE',
        ...
        const db = await getRawDb();
        db.prepare(`DELETE FROM image_processing_log WHERE product_id = ? AND image_url LIKE ?`).run(productId, `%${imageId}%`);
...
// ────────────────────────────────────────────────────────────────────────
router.delete('/api/products/:productId/images', async (req, res) => {
    try {
        const productId = Array.isArray(req.params.productId) ? req.params.productId[0] : req.params.productId;
...
router.post('/api/images/reprocess-edited', upload.single('image'), async (req, res) => {
    try {
        if (!req.file) {
 ...
 */
function extractGcsObjectPath(rawUrl) {
    try {
 ...
 */
async function signGcsObject(objectPath) {
