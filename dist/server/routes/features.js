...
const router = Router();
const VALID_STATUSES = ['new', 'planned', 'in_progress', 'completed', 'declined'];
...
router.post('/api/features', async (req, res) => {
    try {
        ...
        const result = db.prepare(`INSERT INTO feature_requests (title, description, requested_by, priority) VALUES (?, ?, ?, ?)`).run(title.trim(), description.trim(), requested_by || null, prio);
        info(`[Features] New request: "${title.trim().slice(0, 60)}"`);
        const created = db.prepare(`SELECT
        fr.*,
...
router.get('/api/features/:id', async (req, res) => {
    try {
        const db = await getRawDb();
        const id = req.params.id;
        const feature = db.prepare(`SELECT
        fr.*,
        (SELECT COUNT(*) FROM feature_votes fv WHERE fv.feature_id = fr.id) as votes
       FROM feature_requests fr
       WHERE id = ?`).get(id);
...
router.post('/api/features/:id/vote', async (req, res) => {
    try {
        const db = await getRawDb();
        const id = req.params.id;
        ...
        const insert = db.prepare(`INSERT OR IGNORE INTO feature_votes (feature_id, voter_id) VALUES (?, ?)`).run(id, voterId.trim());
        const votesRow = db.prepare(`SELECT COUNT(*) as count FROM feature_votes WHERE feature_id = ?`).get(id);
        res.json({
            ok: true,
...
router.put('/api/features/:id', async (req, res) => {
    try {
        const db = await getRawDb();
        const id = req.params.id;
        ...
        params.push(id);
        const result = db.prepare(`UPDATE feature_requests SET ${updates.join(', ')} WHERE id = ?`).run(...params);
        if (result.changes === 0) {
            res.status(404).json({ error: 'Feature request not found' });
            return;
        }
        const updated = db.prepare(`SELECT
        fr.*,
        (SELECT COUNT(*) FROM feature_votes fv WHERE fv.feature_id = fr.id) as votes
       FROM feature_requests fr
       WHERE id = ?`).get(id);
...
router.delete('/api/features/:id', async (req, res) => {
    try {
        const db = await getRawDb();
        const id = req.params.id;
