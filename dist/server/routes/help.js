...
router.post('/api/help/questions', async (req, res) => {
    try {
        ...
        const result = db.prepare(`INSERT INTO help_questions (question, asked_by, category) VALUES (?, ?, ?)`).run(question.trim(), asked_by || null, category || null);
        ...
        if (aiAnswer) {
            db.prepare(`UPDATE help_questions SET answer = ?, status = 'answered', answered_by = 'AI', updated_at = datetime('now') WHERE id = ?`).run(aiAnswer, questionId);
...
router.get('/api/help/questions', async (req, res) => {
    try {
        const db = await getRawDb();
        const status = (req.query.status || '').trim();
        let questions;
        if (status) {
            questions = db.prepare(`SELECT * FROM help_questions WHERE status = ? ORDER BY created_at DESC`).all(status);
        }
        else {
            questions = db.prepare(`SELECT * FROM help_questions ORDER BY created_at DESC`).all();
        }
...
router.get('/api/help/questions/:id', async (req, res) => {
    try {
        const db = await getRawDb();
...
router.put('/api/help/questions/:id', async (req, res) => {
    try {
        const db = await getRawDb();
        const id = req.params.id;
        ...
        params.push(id);
        const result = db.prepare(`UPDATE help_questions SET ${updates.join(', ')} WHERE id = ?`).run(...params);
        if (result.changes === 0) {
            res.status(404).json({ error: 'Question not found' });
            return;
        }
...
router.delete('/api/help/questions/:id', async (req, res) => {
    try {
        const db = await getRawDb();
        const id = req.params.id;
...
router.get('/api/help/faq', async (_req, res) => {
    try {
        const db = await getRawDb();
        const faq = db.prepare(`SELECT id, question, answer, category, sort_order, updated_at FROM help_questions WHERE status = 'published' ORDER BY category, sort_order ASC, created_at ASC`).all();
...
router.get('/api/help/categories', async (_req, res) => {
    try {
        const db = await getRawDb();
        const cats = db.prepare(`SELECT category, COUNT(*) as count FROM help_questions WHERE status = 'published' AND category IS NOT NULL GROUP BY category ORDER BY category`).all();
        res.json({ data: cats });
    }
    catch (err) {
...
router.get('/api/help/articles', async (req, res) => {
    try {
        const db = await getRawDb();
        const category = (req.query.category || '').trim();
        let articles;
        if (category) {
            articles = db.prepare(`SELECT id, question, answer, category, sort_order, updated_at FROM help_questions WHERE status = 'published' AND category = ? ORDER BY sort_order ASC, created_at ASC`).all(category);
        }
        else {
            articles = db.prepare(`SELECT id, question, answer, category, sort_order, updated_at FROM help_questions WHERE status = 'published' ORDER BY category, sort_order ASC, created_at ASC`).all();
        }
...
router.get('/api/help/articles/:id', async (req, res) => {
    try {
        const db = await getRawDb();
        const id = Number(req.params.id);
        const article = db.prepare(`SELECT id, question, answer, category, sort_order, updated_at FROM help_questions WHERE id = ? AND status = 'published'`).get(id);
        if (!article) {
            res.status(404).json({ error: 'Article not found' });
            return;
        }
        // Get all published articles ordered to find prev/next
        const allArticles = db.prepare(`SELECT id FROM help_questions WHERE status = 'published' ORDER BY category, sort_order ASC, created_at ASC`).all();
