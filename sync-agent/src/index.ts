...
const MANIFEST_PATH = process.env.MANIFEST_PATH ?? path.join(
  process.env.HOME ?? '/tmp',
  '.styleshoots-sync-manifest.json',
);
const LOG_PATH = process.env.LOG_PATH ?? path.join(
  process.env.HOME ?? '/tmp',
  'Library/Logs/styleshoots-sync.log',
);
...
async function uploadFile(localPath: string, gcsKey: string): Promise<boolean> {
  try {
...
async function syncAll(): Promise<{ uploaded: number; skipped: number; errors: number }> {
