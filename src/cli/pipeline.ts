...
export const buildPipelineCommand = () => {
  ...
  pipeline
    ...
    .action(async (productId: string, opts, command) => {
      ...
      try {
        ...
        if (opts.skipDrive) body.skipDrive = true;
  ...
  pipeline
    ...
    .action(async (jobId: string | undefined, _opts, command) => {
      ...
      try {
        if (jobId) {
          ...
          if (command.optsWithGlobals().json) {
            ...
            if (j.error) console.log(`  Error:   ${chalk.red(j.error)}`);
          ...
          if (command.optsWithGlobals().json) {
            ...
            if (!jobs.length) { console.log('No pipeline jobs found.'); return; }
            for (const j of jobs) {
  ...
  pipeline
    ...
    .action(async (jobId: string, _opts, command) => {
    ...
    .action(async (opts, command) => {
      ...
      try {
        ...
        if (command.optsWithGlobals().json) {
          ...
          if (!jobs.length) { console.log('No pipeline history.'); return; }
...
function statusIcon(s: string): string {
  if (s === 'completed' || s === 'success') return chalk.green('✓');
  if (s === 'failed' || s === 'error') return chalk.red('✗');
  if (s === 'running' || s === 'processing') return chalk.yellow('⟳');
